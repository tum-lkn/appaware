...
from .utils_py.util import debug, format_bytes, Logger, getPage, send_json, makeJsonUrl, RateCalc, ProcessStats, CSVFile
...
class TapasPlayer(threading.Thread):
    def __init__(self, controller, parser, media_engine,
        ...
        # init thread
        super(TapasPlayer, self).__init__()
        ...
        self.log_prefix = ''
        self.log_comment = ''
        self.log_segments = None
        self.log_stalling = None
    ...
    def get_metric_logs(self):
        metric_logs = self._metric_logs
        self._metric_logs = []
        return metric_logs

    def _log_metric(self):
        # schedule next metric log metric logging
        reactor.callLater(self._metric_log_interval, self._log_metric)

        metric = {
            "timestamp": time.time(),
            "metric": {
                "quality_level": self.getCurrentLevel(),
                "quality_rate": self.getCurrentRate(),
                "buffer_level": self.media_engine.getQueuedTime(),
                "buffer_size": self.getMaxBufferTime(),
                "segment_index": self.getCurrentSegmentIndex(),
                "segment_max": self.parser.playlists[self.getCurrentLevel()]['end_index'],
                "bandwitdth_estimation": self.getBandwidth()
            }
        }
        self._metric_logs.append(metric)
    ...
    def run(self):
        ...
        # start player
        self.play()
        # run reactor without signal handlers
    ...
    def play(self):
        ...
        def _on_done(res):
            ...
            if self.log_sub_dir:
                ...
                fb = "%s_%d_%%s.csv" % (time.strftime('%Y-%m-%d-%H.%M.%S'), os.getpid())
                self.log_segments = CSVFile(os.path.join(self.log_dir, fb % "segments"))
                self.log_stalling = CSVFile(os.path.join(self.log_dir, fb % "stallings"),
                                            fieldnames=['t_paused', 't_playing', 'getCurrentSegmentIndex'])
    ...
    def fetchNextSegment(self):
        ...
        if self.log_segments:
            self.log_segments.writerow({'seg_idx': self.getCurrentSegmentIndex(), 'ql': self.getCurrentLevel()})
    ...
    def startDownload(self, url, byterange=''):
        ...
        if self.use_persistent_connection:
            ...
            if not self.connection.client:
                return
            if not self.connection.client.isAvailable():
                return
    ...
    def log(self):
        ...
        if self.parser.playlists[0]['end_index'] == \
                self.media_engine.getCurSegment(time.time()) and \
                self.media_engine.getQueuedTime() == 0.0:
            self.stop()
    ...
    def _onStatusChanged(self, media_engine):
        ...
        if media_engine.status == media_engine.PLAYING:
            ...
            if self.log_stalling:
                self.log_stalling.writerow({'t_paused': self.t_paused,
                                            't_playing': t_playing,
                                            'getCurrentSegmentIndex': self.getCurrentSegmentIndex()})

            self.paused_time += t_playing - self.t_paused
        else:
    ...
    def cleanup(self):
        ...
        print("Thread %s: cleanup()" % threading.current_thread())

        if self.log_segments:
            self.log_segments.close()
        if self.log_stalling:
            self.log_stalling.close()

        print("Thread %s: cleanup() finished" % threading.current_thread())
