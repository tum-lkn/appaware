    ...
    def run(self):
        ...
        try:
            while self._running:
                ...
                # send pings every 60s to keep connection up - chekc self._running to avoid ping message flood while stopping clients
                if time_passed >= next_ping and self._running:
                    self._publish(msg={"type": "PING"})
                    next_ping += 60
        except KeyboardInterrupt:
            print("Stopping clinents")
...
if __name__ == '__main__':
    ...
    parser.add_argument("-c", "--config", help="Config file", dest="config", default="/vagrant/configs/demo.json")
    ...
    test_controller = DemoController(config=config, redis_host=args["redis_host"])
