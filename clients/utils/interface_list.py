...
def phy_from_bridge(br):
    """
    Returns a list of physical interfaces of a bridge.

    :param br:
    :return:
    """

    brintfs = [str(i) for i in os.listdir(os.path.join("/sys/devices/virtual/net/", br, "brif"))]

    phy_intfs = []

    for intf in brintfs:
        dev_target = os.readlink(os.path.join("/sys/class/net/", intf))
        if "pci" in dev_target:
            phy_intfs.append(intf)
...
if __name__ == "__main__":
    ...
    for intf in interface_list(external=True, ip=True):
        print("Interface: ", intf)
        if "br" in intf.name:
            print("Phy. bridge members: ", phy_from_bridge(intf.name))
